...
// Cheap pre-filter so a stranger's ordinary DM doesn't trigger a group-admin
// lookup (ownerRouter -> groupResolver.listControllableGroups, a real
// groupFetchAllParticipating call) on every single message. This is NOT a
// security check — just a "does this even look worth checking" gate;
// ownerRouter.tryHandle and toolExecutor still do every real authorization
// check regardless of what matches here.
const GROUP_CONTROL_HINT_RE = /\b(report|status|aria|miss\s*aria|group|admin|anti-?link|lock|unlock|mute|unmute|warn|ban|kick|remove|promote|demote|invite|rename)\b/i;
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const m of messages) {
      try {
        ...
        } else if (!isGroup && text && GROUP_CONTROL_HINT_RE.test(text)) {
          // Not the bot owner, but this looks like it might be a group-admin
          // instruction ("report on X group", "lock the group", "warn
          // John", etc) sent from a DM. ownerRouter.tryHandle does the real
          // work of checking whether this sender actually admins any group
          // the bot also admins — this regex is only here so a stranger's
          // ordinary DM doesn't trigger that group lookup on every message.
          if (await ownerRouter.tryHandle({ sock, chatJid: jid, senderJid, text, agentId, msg: m })) continue;
        }
