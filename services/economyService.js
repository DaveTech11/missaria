...
function getAccount(userId) {
    ...
    if (!db.users[key]) {
        db.users[key] = { aura: CONFIG.startingAura, shards: 0, points: 0, lastSteal: 0, wins: 0, losses: 0 };
        save();
    }
    return db.users[key];
}
...
function getBalance(userId) {
    ...
    return { aura: acc.aura, shards: acc.shards, points: Number(acc.points || 0) };
}


// ------------------------------------------------------------
// AGENT POINTS — used to unlock premium coding for 5 hours
// ------------------------------------------------------------
function getPoints(userId) { return Number(getAccount(userId).points || 0); }
function addPoints(userId, amount) {
    const n = Math.max(0, Math.floor(Number(amount) || 0));
    const acc = getAccount(userId);
    acc.points = getPoints(userId) + n;
    save();
    return { ok: true, points: acc.points, added: n };
}
function spendPoints(userId, amount) {
    const n = Math.max(0, Math.floor(Number(amount) || 0));
    const acc = getAccount(userId);
    if (getPoints(userId) < n) return { ok:false, reason:`Need ${n} points. You have ${getPoints(userId)}.` };
    acc.points = getPoints(userId) - n;
    save();
    return { ok:true, points:acc.points, spent:n };
}
