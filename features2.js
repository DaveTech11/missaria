...
module.exports = function registerFeatures2(bot, ctx) {
  ...
  // Renamed from /song (and /find) to /songinfo — /song and /music are the
  // canonical music search + download commands (commands/music.js), this was
  // colliding with them and firing a second, duplicate reply.
  bot.onText(/^\/(?:songinfo|find)\s+([\s\S]+)/, async (msg, match) => {
    try {
