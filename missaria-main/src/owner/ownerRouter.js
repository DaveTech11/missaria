...
const PARTICIPANT_ACTION_TOOLS = new Set([
  "removeParticipant", "promoteParticipant", "demoteParticipant",
  "warnParticipant", "banParticipant", "muteParticipant", "unmuteParticipant",
]);

const AI_ACTION_DESCRIPTIONS = {
  setAntiLink: "Change anti-link",
  setGroupLocked: "Lock/unlock the group",
  setEditInfoRestricted: "Change who can edit group info",
  updateGroupSubject: "Rename the group",
  updateGroupDescription: "Change the group description",
  revokeGroupInvite: "Revoke the current invite link",
  removeParticipant: "Remove that member",
  promoteParticipant: "Promote that member to admin",
  demoteParticipant: "Demote that member",
  warnParticipant: "Warn that member",
  banParticipant: "Remove and ban that member",
  muteParticipant: "Mute that member",
  unmuteParticipant: "Unmute that member",
};

function describeAiAction(tool) {
  return AI_ACTION_DESCRIPTIONS[tool] || "Do that";
}

/**
 * Resolves which group an AI-classified message is about, same rules as
 * resolveGroupOrAsk (explicit name / "this group" context / short-term
 * memory) but restricted to `candidateGroups` when given — that's how a
 * non-owner admin gets confined to groups they actually control, without
 * ever touching resolveGroup's full, unfiltered group list.
 */
async function resolveGroupForAi(sock, chatJid, senderJid, extracted, candidateGroups) {
  const inScope = (jid) => !candidateGroups || candidateGroups.some((g) => g.jid === jid);
  const scopeHint = candidateGroups ? ` I manage: ${candidateGroups.map((g) => g.subject).join(", ") || "(no groups yet)"}.` : "";

  if (extracted.useContext || !extracted.query) {
    const ctx = getContextGroup(senderJid);
    if (ctx && inScope(ctx.groupJid)) return { jid: ctx.groupJid, subject: ctx.groupSubject };
    if (!extracted.query && candidateGroups && candidateGroups.length === 1) {
      setContext(senderJid, candidateGroups[0].jid, candidateGroups[0].subject);
      return { jid: candidateGroups[0].jid, subject: candidateGroups[0].subject };
    }
    await sock.sendMessage(chatJid, { text: `Which group do you mean?${scopeHint}` });
    return null;
  }

  const result = await resolveGroup(sock, extracted.query, candidateGroups);
  if (result.status === "found") {
    setContext(senderJid, result.group.jid, result.group.subject);
    return { jid: result.group.jid, subject: result.group.subject };
  }
  if (result.status === "ambiguous") {
    const list = result.matches.map((g, i) => `${i + 1}. ${g.subject}`).join("\n");
  ...
  await sock.sendMessage(chatJid, {
    text: candidateGroups
      ? `I don't manage a group matching "${extracted.query}" for you (I need to be admin there, and you need to be admin there too).`
      : `❌ I couldn't find a group matching "${extracted.query}".`,
  });
  return null;
}

/**
 * The AI-understanding entry point. `candidateGroups`: null means "no
 * restriction" (the owner — every group they've resolved into elsewhere
 * still gets rechecked by toolExecutor anyway); an array means "only
 * these groups", used for non-owner group-admins reaching this from a DM.
 */
async function tryHandleAiIntent({ sock, chatJid, senderJid, text, candidateGroups }) {
  const { classify, TOOL_MANIFEST, READ_ONLY_TOOLS } = require("../ai/intentClassifier");
  // The literal rule you asked for: a trailing "?" means this is a
  // question — she'll only look things up, never change anything, no
  // matter how the sentence is phrased. No "?" means it's an instruction,
  // and the full tool set (including actions) is on the table.
  const isQuestion = /\?\s*$/.test(text.trim());
  const tools = isQuestion ? READ_ONLY_TOOLS : TOOL_MANIFEST;

  const result = await classify(text, { tools, isQuestion });
  if (result.error || !result.tool) return false; // nothing confidently recognized — fall through quietly

  const group = await resolveGroupForAi(sock, chatJid, senderJid, { query: result.group_query, useContext: result.use_context_group }, candidateGroups);
  if (!group) return true; // already asked a clarifying/error question

  if (result.clarify_question) {
    await sock.sendMessage(chatJid, { text: result.clarify_question });
    return true;
  }

  let args = { groupJid: group.jid, ...result.args };
  let targetLabel = group.subject;

  if (PARTICIPANT_ACTION_TOOLS.has(result.tool)) {
    const query = result.args.participantQuery;
    if (!query) {
      await sock.sendMessage(chatJid, { text: "Who do you mean? Give me a name or number." });
      return true;
    }
    const meta = await require("../../services/waGroupManager").getGroupMetadata(sock, group.jid);
    if (!meta) {
      await sock.sendMessage(chatJid, { text: `❌ I couldn't load ${group.subject}'s member list.` });
      return true;
    }
    const matches = findParticipantByNameOrNumber(meta.participants, query);
    if (matches.length === 0) {
      await sock.sendMessage(chatJid, { text: `I couldn't find anyone matching "${query}" in ${group.subject}.` });
      return true;
    }
    if (matches.length > 1) {
      const list = matches.slice(0, 10).map((p, i) => `${i + 1}. ${p.name || p.notify || p.id.split("@")[0]}`).join("\n");
      await sock.sendMessage(chatJid, { text: `Multiple members match "${query}" in ${group.subject}:\n${list}\n\nBe more specific.` });
      return true;
    }
    args.participantJid = matches[0].id;
    delete args.participantQuery;
    targetLabel = `${matches[0].name || matches[0].notify || matches[0].id.split("@")[0]} in ${group.subject}`;
  }

  const risk = confirm.riskOf(result.tool);
  if (risk === confirm.RISK.LOW) {
    const toolResult = await toolExecutor.execute(result.tool, { sock, senderJid, groupJid: group.jid }, args);
    await sock.sendMessage(chatJid, { text: formatResult(result.tool, toolResult, targetLabel) });
    return true;
  }
  const promptText = `${describeAiAction(result.tool)} — ${targetLabel}?`;
  confirm.propose(senderJid, result.tool, targetLabel, args, promptText);
...
async function tryHandleImpl({ sock, chatJid, senderJid, text, agentId, msg }) {
  ...
  if (!isOwner(senderJid)) {
    // Not the bot owner. They can still reach the AI-understanding layer,
    // but ONLY for groups where they are personally an admin AND the bot
    // is also admin there — never the owner-only regex cascade below,
    // and never a group outside that list. toolExecutor still re-checks
    // isSenderGroupAdmin for the real group before anything actually
    // runs; this is just what makes it worth trying in the first place.
    const { listControllableGroups } = require("./groupResolver");
    let controllableGroups;
    try {
      controllableGroups = await listControllableGroups(sock, senderJid);
    } catch (err) {
      return false;
    }
    if (controllableGroups.length === 0) return false; // not an admin of any group I also admin — nothing for them here
    const rateCheck = checkAndRecord(senderJid, 20, 60 * 1000);
    if (!rateCheck.allowed) {
      await sock.sendMessage(chatJid, { text: `⏳ Slow down a bit — try again in ${Math.ceil(rateCheck.retryAfterMs / 1000)}s.` });
      return true;
    }
    ...
    if (pending) {
      if (confirm.isAffirmative(t)) {
        ...
        // Non-owner confirmations only ever come from tryHandleAiIntent's
        // GROUP_ADMIN-tool proposals above — never the owner cascade's
        // bulk/media flows — so a plain execute+format is always correct here.
    ...
    return tryHandleAiIntent({ sock, chatJid, senderJid, text: t, candidateGroups: controllableGroups });
  }
  ...
  // Last resort: none of the patterns above matched. Let the AI
  // classifier take a real shot at free-form phrasing ("give me a report
  // on zuno group") before giving up — this never bypasses anything
  // above; it only runs when everything else already said no.
  if (await tryHandleAiIntent({ sock, chatJid, senderJid, text: t, candidateGroups: null })) return true;
...
function formatResult(action, result, label) {
  ...
  switch (action) {
    ...
    case "getGroupParticipants": {
      if (result.data.length === 0) return `No members found in ${label}.`;
      return `👥 ${result.data.length} member(s) in ${label}:\n` + result.data.slice(0, 30).map((p) => `• ${p.name || p.notify || p.id.split("@")[0]}${p.admin ? " (admin)" : ""}`).join("\n") + (result.data.length > 30 ? `\n…and ${result.data.length - 30} more.` : "");
    }
    ...
    case "getGroupSettings":
      return `⚙️ ${label}\nMessaging restricted to admins: ${result.data.messagingRestrictedToAdmins ? "🟢 YES" : "🔴 NO"}\nEditing info restricted to admins: ${result.data.editInfoRestrictedToAdmins ? "🟢 YES" : "🔴 NO"}`;
    case "getGroupReport": {
      const d = result.data;
      return [
        `📊 ${d.subject} — Report`,
        ``,
        `Members: ${d.participantCount} (${d.adminCount} admin${d.adminCount === 1 ? "" : "s"})`,
        ...
        `Warnings on record: ${d.totalWarnings} (across ${d.warnedCount} member${d.warnedCount === 1 ? "" : "s"})`,
      ].join("\n");
    }
    case "warnParticipant": {
      const d = result.data;
      return d.autoKicked
        ? `⚠️ Warned ${label} (${d.count}/${d.limit}) — limit reached, removed automatically.`
        : `⚠️ Warned ${label} (${d.count}/${d.limit}).`;
    }
    case "banParticipant":
      return `🚫 Banned ${label}.`;
    case "unbanParticipant":
      return `✅ Unbanned ${label}.`;
