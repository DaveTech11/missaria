...
const pendingTextConfirm = new Map();

// The callbacks this owner-center module actually manages. It used to gate
// on any callback_data starting with "aria_", but several unrelated public
// buttons share that same prefix — the AI companion's response keyboard
// (aria_regenerate, aria_improve, aria_continue, aria_save_response,
// aria_delete_response, aria_mode_*, aria_memory_open/_toggle,
// aria_clear_history, aria_usage) is available to every user, not just the
// owner. Because Telegram fires every registered "callback_query" listener,
// the blanket prefix check here was intercepting those taps before this
// module's own logic even looked at which button it was, and replying with
// "🔒 Owner access only." underneath the real (working) response. Matching
// only this module's own known tokens fixes that without touching the
// buttons other handlers own.
const OWNER_CALLBACK_EXACT = new Set([
    'aria_intel', 'aria_incidents', 'aria_groups', 'aria_moderation', 'aria_tasks', 'aria_stats',
    'aria_diag', 'aria_audit', 'aria_automod', 'aria_security', 'aria_emergency', 'aria_backup',
    'aria_memory', 'aria_live', 'aria_journal', 'aria_ai', 'aria_message', 'aria_help',
    'aria_cancel_confirm', 'aria_undo', 'aria_backup_create', 'aria_emergency_on', 'aria_emergency_off'
]);
const OWNER_CALLBACK_PREFIXES = [
    'aria_confirm_', 'aria_manage_', 'aria_lock_', 'aria_unlock_', 'aria_linkon_', 'aria_linkoff_',
    'aria_admins_', 'aria_logs_', 'aria_gmod_', 'aria_am_', 'aria_task_', 'aria_restore_'
];
const SCHEDULER_CALLBACK_PREFIXES = [
    'aria_run_', 'aria_toggle_', 'aria_cancel_task_', 'aria_history_', 'aria_retry_'
];
function isOwnerCenterCallback(d) {
    return OWNER_CALLBACK_EXACT.has(d) || OWNER_CALLBACK_PREFIXES.some(p => d.startsWith(p));
}
function isSchedulerCallback(d) {
    return SCHEDULER_CALLBACK_PREFIXES.some(p => d.startsWith(p));
}
...
Automatic moderation is paused for this group.</blockquote>`;
 ...
 bot.onText(/^\/(?:aria|owner)$/i,m=>ownerOnly(m.from.id)?send(m.chat.id,'🌸 <b>MISS ARIA — TELEGRAM OWNER COMMAND CENTER</b>\n\nAI control • groups • moderation • scheduler • analytics • security', {reply_markup:menu()}):denied(m.chat.id));
 ...
 bot.on('callback_query',async q=>{const d=String(q.data||'');if(d==='aria_continue_start'){await bot.answerCallbackQuery(q.id).catch(()=>{});const chat=q.message?.chat?.id||q.from.id;return bot.sendMessage(chat,'/start').catch(()=>{});}if(!isOwnerCenterCallback(d))return;await bot.answerCallbackQuery(q.id).catch(()=>{});const user=q.from.id,chat=q.message?.chat?.id||user;if(!ownerOnly(user))return denied(chat);try{
...
const r=await aiOps.undo(user);return send(chat,r.success?'↩️ <b>Last supported action undone.</b>':`❌ ${esc(r.error.message)}`)}
 ...
 bot.on('message',async m=>{
  ...
  const rawMessage=String(m.text||'').trim();

  // Never let the owner/group controller consume normal bot commands.
  // /start, /menu, /help, /admin and every other slash command belong to
  // their dedicated handlers, not this natural-language controller.
  if (/^\/\w+(?:@\w+)?(?:\s|$)/i.test(rawMessage)) return;

  // In private chats the controller requires an explicit Aria prefix.
  // This prevents casual messages such as "hi", "hello admin", etc. from
  // accidentally entering the Telegram control engine.
  const explicitAria=/^aria(?:[, :.-]|$)/i.test(rawMessage);
  const controllerText=rawMessage.replace(/^aria[, :.-]*/i,'').trim();
  const allowedControllerText=/\b(?:lock|unlock|anti[- ]?link|auto[- ]?mod|mute|unmute|warn|ban|kick|unban|promote|demote|rename|send|message|health|profile|admins|admin|simulate|raid|protection|clean|spam|backup|restart|security|memory|task|reminder|diagnostic|incident)\b/i.test(controllerText);
  if(!allowedControllerText) return;
  if(!groupChat && !explicitAria) return;
  ...
  const raw=controllerText.replace(/^@\w+\s*/,'').trim();
  if(/^\/(start|menu|admin|aria|owner|status|groups|tasks|diagnostics|stats|help|restart)\b/i.test(raw))return;
  ...
  try{
   ...
   intel=text.match(/^(?:enable|turn on|activate) (?:raid protection|raid guard)(?: in| for)?\s+(.+)$/i);if(intel){const gs=await findOneGroup(user,intel[1],m.chat);if(!gs)return send(chat,'❌ I could not uniquely resolve that Telegram group.');const r=await autoMod.configure(user,gs.id,{raid:true});return send(chat,r.success?`🚨 <b>Raid protection ON</b> in ${esc(gs.title)}.\n\nAria will watch sudden join/message spikes and can temporarily lock the group.`:`❌ ${esc(r.error)}`);}
   intel=text.match(/^(?:disable|turn off|deactivate) (?:raid protection|raid guard)(?: in| for)?\s+(.+)$/i);if(intel){const gs=await findOneGroup(user,intel[1],m.chat);if(!gs)return send(chat,'❌ I could not uniquely resolve that Telegram group.');const r=await autoMod.configure(user,gs.id,{raid:false});return send(chat,r.success?`🛡 <b>Raid protection OFF</b> in ${esc(gs.title)}.`:`❌ ${esc(r.error)}`);}
   ...
   if(intent){
    if(intent.type==='intentMeta'){
      if(intent.intentClass==='CONFIRMATION'){
        const token=pendingTextConfirm.get(String(user));
        if(!token)return send(chat,'ℹ️ There is no pending action waiting for confirmation.');
        return executeConfirmed(chat,user,token);
      }
      if(intent.intentClass==='CANCEL'){
        const token=pendingTextConfirm.get(String(user));
        if(token){pendingTextConfirm.delete(String(user));confirmations.consume(token,user);return send(chat,'❌ Pending action cancelled.');}
        return send(chat,'ℹ️ There is no pending action to cancel.');
      }
      if(intent.intentClass==='QUESTION'){
        try{
          const answer=await aiService.generateText({
            prompt:intent.original,
            system:`You are Miss Aria, a Telegram control assistant. The user message has been classified as a QUESTION, not a command. Answer conversationally and helpfully. NEVER execute, simulate, or imply that a Telegram action was performed. If the question asks what an action would do, explain the effect and mention that real actions require authorization and confirmation. Keep the answer concise.`,
            maxTokens:700
          });
          return send(chat,`🧠 ${esc(answer)}`);
        }catch(e){
          return send(chat,'🧠 I understand that as a question, not an action. I could not reach the AI answer service right now.');
        }
      }
      // Unknown/unrecognized intent falls through to the normal help response.
    }
...
${label}`:`❌ ${esc(r.error.message)}`);
 ...
 async function findOneGroup(user,phrase,currentChat){
  const clean = value => String(value||'').toLowerCase().replace(/^@/,'').replace(/[’']/g,'').replace(/\b(group|chat)\b/g,' ').replace(/\s+/g,' ').trim();
  const q = clean(phrase);
  if(!q)return null;

  // When the command is sent inside a Telegram group, prefer that exact
  // current chat when its title matches the requested name. This lets
  // "lock zack group" work even if the group was never cached in state.users.
  if(currentChat && isGroup(currentChat)){
    const currentTitle = clean(currentChat.title);
    const currentRequestedAsThisGroup = /^(?:this|that|here)(?:\s+group)?$/i.test(String(phrase||'').trim());
    const titleMatches = currentTitle && (
      currentTitle === q ||
      currentTitle.includes(q) ||
      q.includes(currentTitle)
    );
    // In a group chat, "lock this group" should target the current chat.
    // For a named target, accept a unique fuzzy match against the current
    // chat even when the group has never been cached in state.users.
    if(currentRequestedAsThisGroup || titleMatches){
      const accessResult = await access.canControl(user,currentChat.id);
      if(accessResult.ok){
  ...
  const gs=await knownGroups(user);
  const exact=gs.filter(c=>{
    const title=clean(c.title), username=clean(c.username);
    return title===q || username===q;
  });
  if(exact.length===1)return exact[0];
  if(exact.length>1)return null;

  const matches=gs.filter(c=>{
